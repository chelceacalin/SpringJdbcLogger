package com.simpleJdbcLogger.Spring.Boot.Jdbc.Logger.common;

public class CommonUtil {
    public static final String CATALOG_NAME = "dto";
    public static final String INSERT_DEPARTMENT = "[InsertDepartment]";
    public static final String INSERT_EMPLOYEES = "[InsertEmployees]";
    public static final String GET_ALL_EMPLOYEES = "[GetAllEmployees]";
    public static final String GET_SETTINGS_BY_KEY = "[GetSettingByKey]";
}
